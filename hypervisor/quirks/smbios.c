...
static void smbios_table_probe(struct acrn_multiboot_info *abi)
{
    void *p;
    EFI_GUID smbios3_guid = SMBIOS3_TABLE_GUID;
    EFI_GUID smbios_guid = SMBIOS_TABLE_GUID;
    uint64_t efi_system_tab_paddr = ((uint64_t)abi->mi_efi_info.efi_systab_hi << 32) | ((uint64_t)abi->mi_efi_info.efi_systab);
...
void try_smbios_passthrough(struct acrn_vm *vm, struct acrn_multiboot_info *abi)
{
    /* For debugging only */
