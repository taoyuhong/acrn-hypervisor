...
static void init_msr_area(struct acrn_vcpu *vcpu)
{
	...
	vcpu->arch.msr_area.guest[vcpu->arch.msr_area.count].msr_index = MSR_IA32_TSC_AUX;
	vcpu->arch.msr_area.guest[vcpu->arch.msr_area.count].value = vcpu->vcpu_id;
	vcpu->arch.msr_area.host[vcpu->arch.msr_area.count].msr_index = MSR_IA32_TSC_AUX;
	vcpu->arch.msr_area.host[vcpu->arch.msr_area.count].value = pcpuid_from_vcpu(vcpu);
	...
	if (is_platform_rdt_capable() && (cfg->clos != hv_clos)) {
		...
		vcpu->arch.msr_area.guest[vcpu->arch.msr_area.count].value = clos2pqr_msr(cfg->clos);
	...
	if (has_ac_for_splitlock() == true) {
		vcpu->arch.msr_area.guest[vcpu->arch.msr_area.count].msr_index = MSR_TEST_CTL;
		vcpu->arch.msr_area.guest[vcpu->arch.msr_area.count].value = msr_read(MSR_TEST_CTL);
		vcpu->arch.msr_area.host[vcpu->arch.msr_area.count].msr_index = MSR_TEST_CTL;
		vcpu->arch.msr_area.host[vcpu->arch.msr_area.count].value = msr_read(MSR_TEST_CTL);
		if (cfg->severity > SEVERITY_SOS) {
			vcpu->arch.msr_area.guest[vcpu->arch.msr_area.count].value &= (~(1U<<29U));
		} else {
			vcpu->arch.msr_area.guest[vcpu->arch.msr_area.count].value |= (1U<<29U);
		}
