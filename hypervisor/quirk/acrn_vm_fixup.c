...
static void *get_acpi_mod_entry(struct acrn_vm *vm, const char *signature, void *acpi)
{
...
static void pheader(const char *str, struct acpi_table_header *header)
{
        pr_err("%s %c%c%c%c size:%d", str, header->signature[0], header->signature[1], header->signature[2], header->signature[3],
                        header->length);
}
...
static void tpm2_fixup(struct acrn_vm *vm)
{
	...
	tpm2 = get_acpi_mod_entry(vm, ACPI_SIG_TPM2, mod->start);
	native = get_acpi_tbl(ACPI_SIG_TPM2);

	pheader(__FUNCTION__, &tpm2->header);
...
void acrn_vm_fixup(struct acrn_vm *vm)
{
